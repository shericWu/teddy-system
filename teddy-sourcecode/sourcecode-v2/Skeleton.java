...
//            SkPolygon2D, SkEdge2D, TrimData, SkVertex2D, 
//            Edge2D, LinkedList, Vector2, NewPolyhedron, 
//            Vertex, Vector3, DrawPanel, Geometry2D, 
//            Def, Polyhedron, Tessellation, ReTessellation, 
//            Vertex2D, Draw3DScene
...
class Skeleton
{
    ...
    private static void propagate(SkPolygon2D skpolygon2d, SkEdge2D skedge2d, TrimData trimdata, LinkedList linkedlist)
    {
        ...
        do
        {
            ...
            if(skpolygon2d2.type != 1)
                break;
            SkEdge2D skedge2d2 = skpolygon2d2.the_other_internal_edge(skedge2d1);
            if(!trimdata.terminals_are_within_this_circle(skedge2d2))
                break;
        ...
        SkEdge2D skedge2d3;
        for(Enumeration enumeration = trimdata.terminal_edges.elements(); enumeration.hasMoreElements(); linkedlist.append(new SkPolygon2D(skedge2d3, skvertex2d1)))
            skedge2d3 = (SkEdge2D)enumeration.nextElement();

    }

    private static void create_polygons_square(NewPolyhedron newpolyhedron, double d, double d1, double d2, double d3, double d4, double d5, double d6, 
    ...
    private static void create_polygons_junction_prepare(Vertex avertex[], double d, double d1, double d2, double d3, double d4, double d5)
    {
        ...
        for(int i = 0; i <= steps; i++)
            avertex1[i] = avertex[steps - i];

        for(int j = 0; j <= steps; j++)
            avertex[j] = avertex1[j];
    ...
    private static void propagate_junction(SkPolygon2D skpolygon2d, LinkedList linkedlist)
    {
        ...
        for(int i = 0; i < skpolygon2d.edges.size(); i++)
        {
            ...
            if(skedge2d1 != skedge2d)
            {
                ...
                if(trimdata1 != null)
                {
                    if(!trimdata1.terminals_are_within_this_circle(skedge2d1))
                        flag = true;
                    else
                        trimdata.merge(trimdata1);
                } else
                {
                    flag1 = true;
                }
    ...
    private static SkEdge2D find_appropriate_edge(LinkedList linkedlist, SkEdge2D skedge2d, SkVertex2D skvertex2d)
    {
        ...
        if(((Edge2D) (skedge2d)).start != skvertex2d)
            vector2.multiple_self(-1D);
        ...
        for(Enumeration enumeration = linkedlist.elements(); enumeration.hasMoreElements();)
        {
            ...
            if(((Edge2D) (skedge2d2)).start != skvertex2d)
                vector2_1.multiple_self(-1D);
    ...
    private static void create_polygons_junction_simple_sub(NewPolyhedron newpolyhedron, double d, double d1, double d2, double d3, double d4, double d5, double d6, 
            double d7, double d8)
    {
        for(int i = 0; i < steps; i++)
        {
    ...
    private static void propagate_junction_final(SkPolygon2D skpolygon2d, LinkedList linkedlist)
    {
        ...
        for(int i = 0; i < skpolygon2d.edges.size(); i++)
        {
            ...
            if(trimdata != null)
                linkedlist1.connect(trimdata.terminal_edges);
        }

        SkVertex2D skvertex2d = skpolygon2d.center;
        SkEdge2D skedge2d;
        for(Enumeration enumeration = linkedlist1.elements(); enumeration.hasMoreElements(); linkedlist.append(new SkPolygon2D(skedge2d, skvertex2d)))
    ...
    private static void create_polygons_junction(NewPolyhedron newpolyhedron, double d, double d1, double d2, double d3, double d4, double d5, double d6, 
            double d7, double d8, double d9, double d10, double d11)
    {
    ...
    private static void create_polygons_external(NewPolyhedron newpolyhedron, double d, double d1, double d2, double d3, double d4, double d5, double d6, 
            double d7, double d8)
    {
        if(d3 == d && d5 == d2)
        ...
        for(int i = -steps; i < steps; i++)
        {
            ...
            if(i == -steps)
                newpolyhedron.add_temp_polygon(d6, d19, d8, d15, d20, d16, d13, d20, d14);
            else
            if(i == steps - 1)
                newpolyhedron.add_temp_polygon(d11, d19, d12, d9, d19, d10, d6, d20, d8);
            else
            if(Vector2.distance(d9, d10, d13, d14) > Vector2.distance(d11, d12, d15, d16))
            {
    ...
    private static void create_polygons_junction_complicated(NewPolyhedron newpolyhedron, double d, double d1, double d2, double d3, double d4, double d5, double d6, 
            double d7, double d8)
    {
        ...
        for(int i = 0; i < steps; i++)
            avertex3[i] = avertex[i];

        for(int j = 0; j < steps; j++)
            avertex3[steps + j] = avertex1[steps - j];

        for(int k = 0; k <= steps; k++)
            avertex3[steps * 2 + k] = avertex2[k];

        LinkedList linkedlist = delauny(avertex3);
        Vertex avertex4[];
        for(Enumeration enumeration = linkedlist.elements(); enumeration.hasMoreElements(); create_polygons_junction_sub(avertex4[0], avertex4[1], avertex4[2]))
            avertex4 = (Vertex[])enumeration.nextElement();
    ...
    private static LinkedList delauny(Vertex avertex[])
    {
        ...
        for(int i = 0; i < steps * 3; i++)
        {
            ...
            if((i <= 0 || !avertex1[2].same_position(avertex[i - 1])) && (i != 0 || !avertex1[2].same_position(avertex[steps * 3 - 1])))
                linkedlist.append(avertex1);
        }
    ...
    private static void create_polygons_junction_simple(NewPolyhedron newpolyhedron, double d, double d1, double d2, double d3, double d4, double d5, double d6, 
            double d7, double d8)
    {
        ...
        do
        {
            ...
            if(skvertex2d != ((Edge2D) (skedge2d)).end)
            {
                ...
                if(linkedlist3.size() > 1)
                    skedge2d2 = find_appropriate_edge(linkedlist3, skedge2d1, skvertex2d);
                else
                    skedge2d2 = (SkEdge2D)linkedlist3.head();
                skedge2d1 = skedge2d2;
            } else
            {
    ...
    public static Polyhedron generate_polyhedron(LinkedList linkedlist)
    {
        ...
        int j = 0;
        for(Enumeration enumeration = linkedlist.elements(); enumeration.hasMoreElements();)
            vertices[j++] = new SkVertex2D((Point)enumeration.nextElement());
        ...
        for(int k = 0; k < i - 1; k++)
        {
            ...
            for(int j1 = 0; j1 < i - 1; j1++)
                if(j1 != k && j1 != k + 1 && (k != i - 2 || j1 != 0) && Geometry2D.left_side(skvertex2d, skvertex2d1, vertices[j1]) && Geometry2D.left_side(get_vertex(j1 - 1), get_vertex(j1), get_vertex(j1 + 1), skvertex2d) && Geometry2D.left_side(get_vertex(j1 - 1), get_vertex(j1), get_vertex(j1 + 1), skvertex2d1))
                {
                    double d6 = (new Vector2(vertices[j1], skvertex2d)).get_cos(new Vector2(vertices[j1], skvertex2d1));
                    if(d6 < d2)
                    {
                        d2 = d6;
            ...
            if(i1 == k + 2 || k == i - 2 && i1 == 1 || k == i - 3 && i1 == 0)
                create_T_triangle(triangles, edges, skvertex2d, skvertex2d1, skvertex2d3);
            else
            if(i1 != k - 1 && (k != 0 || i1 != i - 2))
                create_S_triangle(triangles, edges, skvertex2d, skvertex2d1, skvertex2d3);
        }
        ...
        for(Enumeration enumeration1 = edges.elements(); enumeration1.hasMoreElements();)
        {
            ...
            if(skedge2d.type == 1)
                create_J_triangle(triangles, edges, skedge2d);
        }
        ...
        for(Enumeration enumeration2 = triangles.elements(); enumeration2.hasMoreElements();)
        {
            ...
            {
                LinkedList linkedlist2 = skpolygon2d.get_internal_edges();
                double d = ((SkEdge2D)linkedlist2.head()).length();
                double d3 = ((SkEdge2D)linkedlist2.tail()).length();
                skpolygon2d.height = ((d + d3) / 4D) * Def.GENERATE_HEIGHT;
            } else
        ...
        for(Enumeration enumeration3 = triangles.elements(); enumeration3.hasMoreElements();)
        {
            ...
            if(skpolygon2d1.type == 2)
                if(skpolygon2d1.center_edge == null)
                    skpolygon2d1.center.height = skpolygon2d1.height;
                else
                    skpolygon2d1.center.height = skpolygon2d1.center_edge.height;
        }
        ...
        for(Enumeration enumeration4 = triangles.elements(); enumeration4.hasMoreElements();)
        {
            ...
            if(skpolygon2d2.type == 0)
                propagate_terminal(skpolygon2d2, linkedlist1);
        }
        ...
        for(Enumeration enumeration5 = triangles.elements(); enumeration5.hasMoreElements();)
        {
            ...
            if(skpolygon2d3.type == 2 && !skpolygon2d3.marked)
                propagate_junction_final(skpolygon2d3, linkedlist1);
        }
        ...
        Vertex2D vertex2d3;
        Vertex2D vertex2d4;
        for(Enumeration enumeration6 = linkedlist1.elements(); enumeration6.hasMoreElements(); create_polygons_external(h, ((Vector2) (vertex2d3)).x, 0.0D, ((Vector2) (vertex2d3)).y, ((Vector2) (vertex2d4)).x, 0.0D, ((Vector2) (vertex2d4)).y, ((Vector2) (vertex2d1)).x, d1, ((Vector2) (vertex2d1)).y))
        {
            SkPolygon2D skpolygon2d4 = (SkPolygon2D)enumeration6.nextElement();
            d1 = reverse_convert(skpolygon2d4.center.height);
        ...
        for(Enumeration enumeration7 = triangles.elements(); enumeration7.hasMoreElements();)
        {
            ...
            if(!skpolygon2d5.marked && skpolygon2d5.type == 1)
            {
                ...
                double d4 = reverse_convert(skedge2d3.height);
                ...
                create_polygons_square(h, ((Vector2) (vertex2d6)).x, 0.0D, ((Vector2) (vertex2d6)).y, ((Vector2) (vertex2d8)).x, 0.0D, ((Vector2) (vertex2d8)).y, ((Vector2) (vertex2d11)).x, d4, ((Vector2) (vertex2d11)).y, ((Vector2) (vertex2d12)).x, d8, ((Vector2) (vertex2d12)).y);
                create_polygons_internal(h, ((Vector2) (vertex2d12)).x, d8, ((Vector2) (vertex2d12)).y, ((Vector2) (vertex2d11)).x, d4, ((Vector2) (vertex2d11)).y, ((Vector2) (vertex2d10)).x, 0.0D, ((Vector2) (vertex2d10)).y);
            } else
            if(!skpolygon2d5.marked && skpolygon2d5.type == 2)
            {
                SkVertex2D skvertex2d2 = skpolygon2d5.center;
                Vertex2D vertex2d = reverse_convert(skvertex2d2);
                double d5 = reverse_convert(skvertex2d2.height);
                for(int l = 0; l < skpolygon2d5.edges.size(); l++)
                    if(skpolygon2d5.trimData[l] == null && skpolygon2d5.get_edge(l) != skpolygon2d5.center_edge)
                    {
                        ...
                        create_polygons_junction(h, ((Vector2) (vertex2d5)).x, 0.0D, ((Vector2) (vertex2d5)).y, ((Vector2) (vertex2d7)).x, 0.0D, ((Vector2) (vertex2d7)).y, ((Vector2) (vertex2d9)).x, d10, ((Vector2) (vertex2d9)).y, ((Vector2) (vertex2d)).x, d5, ((Vector2) (vertex2d)).y);
                    }
    ...
    private static Vertex delauny_sub(Vertex avertex[], Vertex vertex, Vertex vertex1)
    {
        ...
        byte byte0 = -100;
        for(int k = 0; k < i - 1; k++)
            if(avertex[k] != vertex && avertex[k] != vertex1)
            {
    ...
    private static void create_polygons_internal(NewPolyhedron newpolyhedron, double d, double d1, double d2, double d3, double d4, double d5, double d6, 
            double d7, double d8)
    {
        if(d == d3 && d2 == d5 && d1 == d4)
            System.out.println("clashed internal");
        for(int i = -steps; i < steps; i++)
        {
            ...
            double d16 = d5 + (d8 - d5) * d18;
            if(i == -1)
                newpolyhedron.add_temp_polygon(d6, 0.0D, d8, d11, d20, d12, d9, d19, d10);
            else
            if(i == 0)
                newpolyhedron.add_temp_polygon(d15, d21, d16, d13, d22, d14, d6, 0.0D, d8);
            else
            if(Vector2.distance(d9, d10, d13, d14) > Vector2.distance(d11, d12, d15, d16))
            {
                newpolyhedron.add_temp_polygon(d15, d21, d16, d13, d22, d14, d11, d20, d12);
